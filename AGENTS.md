...
| 1 | GUI live sensor (Tauri, ganti GPUI 2026-09-14) | ✅ COMPILE HIJAU + runtime stabil; tab Dashboard/Performance/Fan/Keyboard/Power/Settings, gauge, chart kurva + editor drag, kartu GPU/Mem/Baterai/RAPL, pill EC; state via zustand persist (`tauri-app/src/lib/store.ts`); tray icon + autostart login (`tauri-plugin-autostart`, `--minimized`, close-to-tray) — `cargo check` hijau, `tsc` hijau; CATATAN LOGIN (2026-09-16): Hyprland tak menjalankan XDG autostart sendiri → wajib `systemctl --user add-wants graphical-session.target xdg-desktop-autostart.target`; Exec .desktop HARUS stabil `~/.local/bin/axioo-center-autostart` (wrapper resolve AppImage terbaru di `~/Applications`, karena AppImageLauncher memindah/rename AppImage tiap integrate/update sehingga path hardcoded basi + generator skip "executable does not exist"); toggle autostart di Settings/tray (plugin) menulis Exec path mentah ber-spasi yang generator TOLAK — habis toggle ON, terapkan ulang file `~/.config/autostart/axioo-center.desktop` dari wrapper (fix permanen: command autostart wrapper-aware di backend + rebuild AppImage, belum dikerjakan) |
